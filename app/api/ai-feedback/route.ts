...
export async function POST(request: Request) {
  ...
  let body: { practiceItemId?: string; answerTitle?: string; answerText?: string };
  try {
  ...
  const prompt = `你是一名严谨的HSK 6写作缩写教师。比较原文与学习者缩写，按照HSK 6写作任务要求给出具体、可执行的中文反馈。
...
- titleFeedback、lengthFeedback、fidelityFeedback和viewpointFeedback只写直接、自然的检查结论。结论已经清楚时立即结束，不要追加“这一项没有明显问题”“符合相关要求”“整体表现良好”等重复评价。例如只写“当前答案没有加入个人观点或评价。”
- 上述四项检查必须避免含糊措辞：发现问题就具体指出；没有发现个人观点时直接写“当前答案没有加入个人观点或评价。”，不要写“暂未发现明显问题”。
...
${answerText}`;
  ...
  if (answerCharCount < targetCharCount * 0.5) {
    const issue = `正文只有${answerCharCount}字，远低于约${targetCharCount}字的任务要求，尚未形成完整缩写。`;
    ...
    const issue = `正文约${answerCharCount}字，明显超过约${targetCharCount}字的任务要求，需要进一步压缩。`;
    feedback.priorityIssues = [issue, ...feedback.priorityIssues.filter((item) => !item.includes("字"))];
    feedback.lengthFeedback = issue;
  }
  if (!answerTitle) {
