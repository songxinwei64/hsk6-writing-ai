...
const typeDetails = {
  ...
  mock: { label: "HSK 6 Mock Test", href: "/practice/mock" },
} as const;
...
export default async function AttemptDetailPage({ params }: { params: Promise<{ attemptId: string }> }) {
  ...
  const label = details?.label ?? "Writing Practice";
  const completedAt = attempt.completed_at ?? attempt.updated_at;

  return (
    <main className="page">
      <article className="attempt-detail-shell">
        <Link className="back-link" href="/my-library">← Back to My Practice</Link>
        ...
        <div className="attempt-reference-grid">
          ...
          <section className="attempt-detail-section">
            <span>Key Point</span>
            <p>{item.explanation || "Compare the original passage and suggested answer, then check whether the main people, events, and outcome are complete."}</p>
          </section>
        </div>
        <div className="attempt-detail-actions">
          <Link href={details?.href ?? "/practice"}>Continue Practicing</Link>
          <Link href="/my-library">View All Records</Link>
        </div>
      </article>
    </main>
  );
